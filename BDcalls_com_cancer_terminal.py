...
def Portero_entrada(placa, nova_km):
    try:
        ...
        cursor.execute(
            "UPDATE registro_entrada SET KMEntrada = %s, HoraEntrada = %s WHERE idVeiculo = %s AND idRegistroSaida = %s AND (KMEntrada IS NULL OR HoraEntrada IS NULL)",
            (nova_km, hora_chegada, id_veiculo, id_saida)
        )
